...
module FastMixAndQuench
  ...
  def arrange_tubes(args)
    ...
    show do
      ...
      note 'Leave space between the groups of tubes' if args[:grouped_ops].keys.length > 1

      h1 = args[:treatment_name].titlecase
      h2 = args[:sample_name].titlecase

      args[:grouped_ops].each do |_bfr, ops|
        note '<br>'
  ...
  def incubate(args)
    ...
    show do
      ...
      note "Incubate #{qty_display(args[:incubation_time])}"
      note temp_instructions(args[:temp])
      note incubate_in_dark if args[:incubate_in_dark]
      ...
      if args[:quench_protease]
        ...
        args[:grouped_ops].each do |_bfr, ops|
          note ''
          table wash_buffer_table(ops, output_handle: args[:output_handle],
