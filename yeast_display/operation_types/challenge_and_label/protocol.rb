...
class Protocol
  ...
  include LabelWithAntibody
  include PrepareTreatmentDilutions
  include NormalizeCultureDensity
  include WashYeast
  include FastMixAndQuench
  ...
  def main
    ...
    wash_out_protease if @plan_params[:n_protease_washes] > 0

    label_with_antibody if @plan_params[:incubate_with_antibody]

    wash_out_antibody if @plan_params[:n_antibody_washes] > 0
  ...
  def update_plan_params
    ...
    associate_plan_options(plan_params) if debug && MY_DEBUG
  end
  ...
  def calculate_volumes
    ...
    @plan_params[:antibody_microliters] =
      @plan_params[:antibody_microliters_per_od_ml] * @plan_params[:od_ml_needed]
  ...
  def add_buffers(op, reagent, handle)
    ...
    buffer = if sample_id
               ...
               reagent.properties[handle.to_s]
             end

    return unless buffer

    container = buffer_container(buffer: buffer, handle: handle)
    op.add_input(handle, buffer, container)
  end
