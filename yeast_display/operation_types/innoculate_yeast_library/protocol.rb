...
class Protocol
  ...
  def main
    ...
    group_ops_by_container.each do |container, container_group|
      container_group.retrieve(only: [INPUT_YEAST]).make
    ...
    glycerol_stocks.map(&:mark_as_deleted)

    show do
      title 'Throw Away Glycerol Stocks'

      note "Throw away all the <b>empty</b> glycerol stocks (#{glycerol_stocks.map(&:to_s).to_sentence})."
    end
    ...
    operations.each do |op|
      unless op.input(INPUT_YEAST).item.get(:bin).nil? # sorted sample
